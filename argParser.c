...
void arg_printHelp(ArgList_t *arg)
{
  ArgParam_t *param;
  ArgSwitch_t *sw;
  printf("Paramters:\n");
  for(param = lst_getFirst(arg->argReg); param; param = lst_getNext(arg->argReg))
  {
    if(param->description)
    {
      printf("\t-%c <var>/ --%s <var>\n\t\t%s\n", param->name, param->lname, param->description);
    }
  }
  printf("Switches:\n");
  for(sw = lst_getFirst(arg->switchReg); sw; sw = lst_getNext(arg->switchReg))
  {
    if(sw->description)
    {
      printf("\t-%c / --%s\n\t\t%s\n", sw->name, sw->lname, sw->description);
    }
  }
}
 ...
 */
ArgSwitch_t *arg_addSwitch(ArgList_t *arg, char name, char *lname, char *desc)
{
  if((!isalnum(name)) || (!lname && !strlen(lname))) return NULL;
  ...
  if(desc)
  {
    tmp->description = malloc(strlen(desc)+1);
...
void arg_destroySwitch(ArgSwitch_t *sw)
{
  ...
  if(sw->description) free(sw->description);
  free(sw);
}
...
void arg_destroyParam(ArgParam_t *parm)
{
  ...
  if(parm->description) free(parm->description);
 ...
 * @return pointer to created struct to be used later after parsing
 */
ArgParam_t  *arg_addParam(ArgList_t *arg, char name, char *lname, char *desc)
{
  if((!isalnum(name)) || (!lname && !strlen(lname))) return NULL;
